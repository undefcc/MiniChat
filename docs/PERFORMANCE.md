...
## 03. 状态上云：站点注册从内存迁移到 Redis (2026-02-06)

### 🧯 痛点回放
随着信令服务准备做多实例部署，我们发现原本的“站点注册表”保存在单个进程内存里，导致：
*   重启即丢：服务重启后在线列表瞬间清空，前端看起来像“全站离线”。
*   多实例不一致：A 实例注册的站点，B 实例查不到，路由失败。

### 🧱 改造方案
把站点注册表与在线列表迁移到 Redis，并用原子化管道写入：
*   **映射关系**：
    *   `signaling:station:{stationId}:socket` -> `socketId`
    *   `signaling:station:sock:{socketId}` -> `stationId`
*   **站点元信息**：`signaling:station:{stationId}:meta` (JSON)
*   **在线集合**：`signaling:stations:online` (Set)
*   **过期兜底**：Key 统一设置 24h TTL，防止僵尸数据

`StationService` 通过 `pipeline` 一次写入这些 Key，保证性能和一致性；`StationGateway` 在断开时清理 Redis 映射，保证在线列表准确。

### ✅ 收益
*   **多实例共享**：站点在线状态全局一致，路由命中率提升到稳定水平。
*   **重启无感**：服务滚动更新时，在线状态不再“闪断”。
*   **可观测更清晰**：在线列表一目了然，可直接在 Redis 查看与排障。
